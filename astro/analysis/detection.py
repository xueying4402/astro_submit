...
class Detection:
    ...
    def detect_tod_read_write_dependencies(self, primary_function, primary_sdg, matched_function, matched_sdg, composed_sdg, attack_node, all_predecessors, count):
        ...
        if len(dependencies['dependencies']) != 0:
            ...
            if not config.get_scale():  self._log.info("TOD dependency detected in file %s in contract %s in by composing functions %s and %s" % (self._contract_name, primary_function.contract.name, primary_function.name, matched_function.name))
    ...
    def detect_dao_read_write_dependencies(self, primary_function, primary_sdg, matched_function, matched_sdg, composed_sdg, call_node, all_predecessors, count):
        ...
        if len(dependencies['dependencies']) != 0:
            self.global_detection[self._contract_name].append(dependencies)
            if not config.get_scale():  self._log.info("DAO dependency detected in file %s in contract %s in by composing functions %s and %s" % (self._contract_name, primary_function.contract.name, primary_function.name, matched_function.name))
