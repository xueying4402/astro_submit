...
def add_edge_from_nodes(graph, src_nodes, dest_node, key='N', ref='N', de_entrypoint=False, icfg=None):
    if de_entrypoint:
        max_before = 0
        for src_node in src_nodes:
            if src_node._block_id > max_before:
                max_before = src_node._block_id
        min_after = dest_node._block_id
        diff = max_before + 1 - min_after
        for node in graph.nodes():
            if icfg.has_node(node):
                node._block_id += diff
        for src_node in src_nodes:
            graph.add_edge(src_node, dest_node, key=key, ref=ref)
        return
