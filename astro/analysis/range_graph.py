...
class VRG:
    ...
    def setup(self):
        ...
        for leaf_node in leaf_nodes:
            ...
            if type(function).__name__ == 'StateVariableSolc' or type(function).__name__ == 'StateVariable':
                continue
    ...
    def check_return_summary(self, cfg_obj):
        if len(cfg_obj._return_summary) != 0:
            print("=======================================================================")
            if not config.get_scale():  self.log.info("Analysing return summary for function %s" %cfg_obj._function.name)
    ...
    def return_summary_handler(self, cfg_obj):
        ...
        if not config.get_scale():  self.log.info("Generating return summary for %s"  % cfg_obj._function.name)
        generate_return_summary(self.log, self, cfg_obj, parameters, return_values)
        if not config.get_scale():  self.log.info("Return summary generated for %s"  % cfg_obj._function.name)
    ...
    def generate_range_graph(self, cfg_obj, parameters, return_values):
        for ircall in cfg_obj._ircall_to_bb.keys():
            ...
            if type(ircall.function).__name__ == 'StateVariableSolc' or type(ircall.function).__name__ == 'StateVariable':
                continue
        ...
        if cfg_obj in self.value_graph.keys():
            ...
            if not config.get_scale():  self.log.info("Range graph generated")
        
        else:
            if not config.get_scale():  self.log.info("State variable assignement do not exist")
        
        if not config.get_scale():  self.log.info("Processed function %s"  % cfg_obj._function.name)
        ...
        for parent in parents:
            ...
            if flag_condition:
                ...
                if argument != None:
                    ...
                    if type(argument).__name__ == 'LocalVariableSolc' or type(argument).__name__ == 'LocalVariable':
    ...
    def process_rvalues(self, cfg_obj, s_var, container_bb, rvalue, parameters, svar_assign_ir, return_values):
        ...
        elif type(rvalue).__name__ == 'LocalVariableSolc' or type(rvalue).__name__ == 'LocalVariable':
        ...
        elif type(rvalue).__name__ == 'StateVariableSolc' or type(rvalue).__name__ == 'StateVariable':
