...
TEMPVAR_TYPES = ['LocalVariableSolc', 'LocalVariable', 'TemporaryVariable']
class SDG:
    """
    ...
    def find_dependent_statevars(self, dep_vars):
        state_var_dep_list = []
        for var in dep_vars:
            if type(var).__name__ == 'StateVariableSolc' or type(var).__name__ == 'StateVariable':
        ...
        try:
            ...
            for instr in instrs:
                ...
                if type(instr).__name__ == 'Index':
                    if type(instr.variable_left).__name__ == 'StateVariableSolc' or type(instr.variable_left).__name__ == 'StateVariable':
                        lvalue = instr.lvalue
                ...
                # where store the structure var and the member var
                    ...
                    if instr.lvalue not in self._ref_to_state.keys():
                        if type(var_left).__name__ == 'StateVariableSolc' or type(var_left).__name__ == 'StateVariable':
    ...
    def find_dependent_vars(self, contract, function, dep_vars):
        ...
        for dep_var in dep_vars:
            if type(dep_var).__name__ == 'ReferenceVariable':
                ...
                if dep_var not in CFG.member_ir_var_left and type(dep_var.points_to_origin).__name__ != 'LocalVariableSolc' or type(dep_var.points_to_origin).__name__ != 'LocalVariable':
    ...
    def add_store_edge(self, contract, graph, instr_block, state_var_w):
        ...
        if type(state_var_w).__name__ == 'StateVariableSolc' or type(state_var_w).__name__ == 'StateVariable':
        ...
        for var_w in s_var_list:
            ...
            if type(var_w).__name__ == 'StateVariableSolc' or type(var_w).__name__ == 'StateVariable':
        ...
        try:
            if len(variables) > 0:
                ...
                for s_var in common_vars:
                    ...
                    if type(s_var).__name__ == 'StateVariableSolc' or type(s_var).__name__ == 'StateVariable':
    ...
    def build_sdg(self, contract, function, sicfg):
        ...
        for instr_block in sicfg.nodes:
            ...
            if type(ir_instr).__name__ == "Binary" or type(ir_instr).__name__ == 'Assignment' or type(ir_instr).__name__ == 'Unary':
                ...
                if type(lvalue).__name__ == 'ReferenceVariable' or  type(lvalue).__name__ == 'StateVariableSolc' or type(lvalue).__name__ == 'StateVariable':
    ...
    def do_inter_contract_analysis(self, contract_address, destination_function):
        ...
        if not config.get_scale():  self._log.info('Interprocedural CFG generation started!')
        generated_icfg, icfg_objects = imported_package.generate_icfg(slither_obj, None, self._result_dir, self._dump_graph, self._log)
        if not config.get_scale():  self._log.info('Interprocedural CFG generation finished!')

        # Build storage dependency graph
        if not config.get_scale():  self._log.info('Storage dependency graph generation started!')
        ...
        if not config.get_scale():  self._log.info('Storage dependency graph generation finished!')
    ...
    def get_conditional_dependencies(self, instr_block, sicfg, target, dependent_vars, visited_instrs):
        ...
        elif type(target).__name__ == 'LocalVariableSolc' or type(target).__name__ == 'LocalVariable' or type(target).__name__ == 'TemporaryVariable'\
        ...
        for instr in basic_block._instructions:
            if type(instr).__name__ != 'Node':
                ...
                for var_used in vars_used:
                    ...
                    if type(var_used).__name__ == 'StateVariableSolc' or type(var_used).__name__ == 'StateVariable':
                    ...
                    if type(var_used).__name__ == 'ReferenceVariable':
                        if var_used not in CFG.member_ir_var_left and type(var_used.points_to_origin).__name__ != 'LocalVariableSolc' or type(var_used.points_to_origin).__name__ != 'LocalVariable':
    ...
    def get_root_basic_block(self, graph):
        ...
        for node in nodes:
            ...
            if instr_type.type == NodeType.ENTRYPOINT:
                root_node = node
        return root_node
