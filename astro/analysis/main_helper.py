...
def generate_icfg(slither_obj, callgraph, graph_dir, dump_graph, log):
    ...
    for contract in slither_obj.contracts:
        for modifier in contract.modifiers:
            if not config.get_scale():  info('Interprocedural CFG generation started for modifier %s' % modifier.name)
            modifier_cfg = ICFG(slither_obj, contract, modifier, callgraph, graph_dir, dump_graph, log)
            if not config.get_scale():  info('Interprocedural CFG generated for modifer %s' % modifier.name)
    ...
    for contract in slither_obj.contracts:
        for function in contract.functions:
            ...
            if (config.get_scale() or compile_config.get_xblock()) and config.skip_dump_file(function.name):
                continue
            if not config.get_scale():  info('Interprocedural CFG generation started for %s' % function.name)
            function_cfg = ICFG(slither_obj, contract, function, callgraph, graph_dir, dump_graph, log)
            if not config.get_scale():  info('Interprocedural CFG generated for %s' % function.name)
...
def do_inter_contract_analysis(contract_path, call_instr, destination_function, log, result_dir):
    ...
    if not config.get_scale():  log.info('Storage dependency graph generation started!')
    generated_sdg, sdg_objects, owner_only_modifiers = generate_sdg(slither_obj, generated_icfg, result_dir, False, log)
    if not config.get_scale():  log.info('Storage dependency graph generation finished!')
...
def reset_static_state():
    ...
    State.function_cfg = {}
    State.lvalue_vars = {}
    State.member_ir_var_left = []
    State.refvar_names = {}
    State.contract_enums = {}

    ## ICFG

    State.icfg_generated = {}
    State.locals_to_declare = {}
    State.icfg_objects = {}
    State.function_to_graph = []
    State.uid = 0

    ## SDG
    State.sdg_generated = {}
    State.contract_vars = {}
    State.map_svars_to_struct = {}
    State.map_svars_to_sobj = {}
    State.var_node_list = []
    State.inter_contract_analysis = {}
...
def analyze_call_destination(sdg_obj, vrg_obj, ir_instr, function, owner_only_vars, blacklisted_functions):
    ...
    if type(call_destination).__name__ == 'StateVariableSolc' or type(call_destination).__name__ == 'StateVariable':
