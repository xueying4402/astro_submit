...
def get_solc_path_from_version(solc_version, log=None, lookup_if_unavailable=False):
    ...
    try:
        ...
        if log:
            if not config.get_scale():  log.info('solc %s found at %s' % (solc_version, solc_path))
        return solc_path
    except KeyError:
        if lookup_if_unavailable:
            ...
            try:
                ...
                if not config.get_scale():  log.info('solc %s found as an alternative' % solc_version)
                solc_path = AVAILABLE_SOLC_VERSION_PATHS[solc_version]
                if not config.get_scale():  log.info('solc %s found at %s' % (solc_version, solc_path))
                return solc_path
            except SolcVersionUnavailableException:
                if log:
                    if not config.get_scale():  log.info('No suitable patch version found')
                raise SolcVersionUnavailableException(solc_version)
        else:
            if log:
                if not config.get_scale():  log.info('solc %s unavailable' % solc_version)
            raise SolcVersionUnavailableException(solc_version)
...
def get_solc_path(contract_path, log=None, use_solc_max=False):
    ...
    if log:
        if not config.get_scale():  log.info('Version constraints: %s' % str(simple_version_strings))
    ...
    if log:
        if not config.get_scale():  log.info('solc version: %s' % solc_version)
        
    use_solc_select_path = False
    if solc_version[0] == "v":
        solc_version = solc_version[1:]
        use_solc_select_path = True
        
    if int(solc_version[2]) < 4:
        raise RuntimeError("Invalid version - only solc versions above '0.4.0' are available")
    
    if use_solc_max:
        solc_version_base = solc_version[2]
        solc_version = SOLC_MAX_VERSIONS.get(solc_version_base)
        
    if solc_version == None:
        raise RuntimeError("wrong solc_version: ", solc_version_base)
    ...
    if use_solc_select_path or os.path.isdir(solc_path):
        solc_path = os.path.join(solc_path, solc_binary)
...
def compile_solidity(contract_path, output_dir, args=['--bin'], solc_version=None, analysis_console_output_path=None, log=None):
    ...
    if log:
        if not config.get_scale():  log.info('Command: %s' % cmd_string)
